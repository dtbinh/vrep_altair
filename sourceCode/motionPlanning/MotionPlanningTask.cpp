...
//	_data->setData(_jointHandles.size(),&_jointHandles[0],&_jointStepCount[0],_ikGroupId,basef->getID(),tipf->getID(),_robotSelfCollEntity1,_robotSelfCollEntity2,_robotEntity,_obstacleEntity,_distanceThreshold,_tipMetric,&_robotMetric[0]);
	CPathPlanningInterface::setMpObjectData(_data,_jointHandles.size(),&_jointHandles[0],&_jointStepCount[0],_ikGroupId,basef->getID(),tipf->getID(),_robotSelfCollEntity1,_robotSelfCollEntity2,_robotEntity,_obstacleEntity,_distanceThreshold,_tipMetric,&_robotMetric[0]);
	if (existing)
	{
		if (collidingNodeCnt!=NULL)
...
//			totalNodeCnt[0]=_data->getPhase1NodeCnt(0);
			totalNodeCnt[0]=CPathPlanningInterface::getMpPhase1NodeCnt(_data,0);
		if (outputActivityToConsole)
			printf("Finished preparing the motion planning data structure. CollidingNodes: %i / %i\n",collidingNodeCnt[0],totalNodeCnt[0]);
		return(true);
	}
	if (outputActivityToConsole)
...
void CMotionPlanningTask::_saveAllSceneJointPositionsAndModes(std::vector<CJoint*>& joints,std::vector<float>& jointPositions,std::vector<int>& jointModes)
{
	joints.clear();
	jointPositions.clear();
	jointModes.clear();
...
void CMotionPlanningTask::_restoreAllSceneJointPositionsAndModes(const std::vector<CJoint*>& joints,const std::vector<float>& jointPositions,const std::vector<int>& jointModes)
{
	for (int i=0;i<int(joints.size());i++)
	{
		...
		if (joints[i]->getJointMode()!=jointModes[i])
			joints[i]->setJointMode(jointModes[i]);
	}
}
...
//		float* pathData=_data->findPath(startConfig,goalConfig,options,stepSize,outputConfigsCnt,maxTimeInMs,auxIntParams,auxFloatParams);
		...
		if (pathData!=NULL)
		{
			int cnt=outputConfigsCnt[0]*_jointHandles.size()+outputConfigsCnt[0]+outputConfigsCnt[0]*3;
