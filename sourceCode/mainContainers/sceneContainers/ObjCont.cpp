...
{ // Actualize the lights positions
	App::ct->environment->activateAmbientLight();
	...
	for (int i=0;i<int(lightList.size());i++)
	{
		...
		if ( ((!useLocalLights)&&(!LocalLight))||(useLocalLights&&LocalLight&&light->isObjectParentedWith(viewable)) )
		{
			if ((light->getLightActive())&&(activeLightCounter<CIloIlo::maximumOpenGlLights))
			{
				...
				float ambientLight[4]={light->lightColor.colors[0],light->lightColor.colors[1],
										light->lightColor.colors[2],1.0f};
				float diffuseLight[4]={light->lightColor.colors[3],light->lightColor.colors[4],
										light->lightColor.colors[5],1.0f};
				float specularLight[4]={light->lightColor.colors[6],light->lightColor.colors[7],
										light->lightColor.colors[8],1.0f};
