		...
		IF_UI_EVENT_CAN_WRITE_DATA_CMD("IMPORT_MESH_CMD")
		{
			...
			for (int i=0;i<int(filenamesAndPaths.size());i++)
			{
				...
				if (VFile::doesFileExist(filenameAndPath))
				{
					...
					if (commonImportRoutine(fileFormat,filenameAndPath,showDlg,scalingFactor))
						App::uiThread->addStatusLine(IDSNS_DONE);
					else
...
bool CFileMenu::commonImportRoutine(int importType,const std::string& pathName,bool showDlg,float& sf)
{
	FUNCTION_DEBUG;
	...
	for (int i=0;i<importer->getGroupCount();i++)
	{
		...
		{ 
			...
			CMeshManip::checkVerticesIndicesNormalsTexCoords(*vert,*ind,norm,tex,App::userSettings->identicalVerticesCheck,App::userSettings->identicalVerticesTolerance/sf,App::userSettings->identicalTrianglesCheck);
	...
	bool forbidDecomposition=false;
	...
	{ // We have too many lose elements... we merge them
		...
		CMeshManip::checkVerticesIndicesNormalsTexCoords(aVertices[0][0],aIndices[0][0],aNormals[0],aTextureCoordinates[0],App::userSettings->identicalVerticesCheck,App::userSettings->identicalVerticesTolerance,App::userSettings->identicalTrianglesCheck);
		...
		if (aMaterials[i]!=NULL)
		{
			...
			{ // Texture:
				...
				if (VFile::doesFileExist(tmp))
				{
					...
					int resX,resY,n;
					unsigned char* data=CImageLoader::load(tmp.c_str(),&resX,&resY,&n,0,scaleTexturesTo);
					bool rgba=(n==4);
					if (n<3)
					{
						delete[] data;
...
bool CFileMenu::pathImportRoutine(const std::string& pathName)
{
	if (VFile::doesFileExist(pathName))
	{
		try
		{
			...
			// We read:
			// (x,y,z), (a,b,g), velocityRatio, bezierPtCount,beforeRatio,afterRatio
			while (archive.readLine(currentPos,line,false))
			{
				...
				int validLevel=0; // nothing read
				if (cnt>2)
					validLevel=1; // position
				if (cnt>5)
					validLevel=2; // euler angles
				if (cnt>6)
					validLevel=3; // velocity ratio
				if (cnt>7)
					validLevel=4; // bezier count
				if (cnt>=10)
					validLevel=5; // ratio before and after
				if (cnt>=3)
				{ // We have a point!
					readData.push_back(data[0]); // X
					readData.push_back(data[1]); // Y
					readData.push_back(data[2]); // Z
					if (cnt>=6)
					{
						automaticOrientation=false;
						readData.push_back(data[3]*degToRad); // Euler 0
						readData.push_back(data[4]*degToRad); // Euler 1
						readData.push_back(data[5]*degToRad); // Euler 2
						if (cnt>=7)
						{
							readData.push_back(data[6]); // velocity ratio
							if (cnt>=8)
							{
								readData.push_back(data[7]); // Bezier count
								if (cnt>=10)
								{
									readData.push_back(data[8]); // Bezier 0
									readData.push_back(data[9]); // Bezier 1
								}
								else
								{ // default values here
			...
			if (readData.size()!=0)
			{
				...
				for (int i=0;i<int(readData.size())/10;i++)
				{
					CSimplePathPoint* it=new CSimplePathPoint();
					C7Vector tr;
					tr.X(0)=readData[10*i+0];
					tr.X(1)=readData[10*i+1];
					tr.X(2)=readData[10*i+2];
					tr.Q.setEulerAngles(readData[10*i+3],readData[10*i+4],readData[10*i+5]);
					it->setTransformation(tr,newObject->pathContainer->getAttributes());
					it->setMaxRelAbsVelocity(readData[10*i+6]);
					it->setBezierPointCount(int(readData[10*i+7]+0.5f));
					it->setBezierFactors(readData[10*i+8],readData[10*i+9]);
	...
	try
	{
		...
		if (bezierPoints)
		{
			for (int i=0;i<it->getBezierPathPointCount();i++)
			{
				...
				std::string line(tt::FNb(2,tr.X(0),6)+',');
				line+=tt::FNb(2,tr.X(1),6)+',';
				line+=tt::FNb(2,tr.X(2),6)+',';
				line+=tt::FNb(3,euler(0)*radToDeg,6)+',';
				line+=tt::FNb(3,euler(1)*radToDeg,6)+',';
				line+=tt::FNb(3,euler(2)*radToDeg,6)+',';
				line+=tt::FNb(0,bp->getMaxRelAbsVelocity(),6,false);
				ar.writeLine(line);
			}
		}
		else
		{
			for (int i=0;i<it->getSimplePathPointCount();i++)
			{
				...
				line+=tt::FNb(3,euler(2)*radToDeg,6)+',';
				line+=tt::FNb(0,bp->getMaxRelAbsVelocity(),6,false)+',';
				line+=tt::FNb(3,bp->getBezierPointCount(),false)+',';
				line+=tt::FNb(0,f0,3,false)+',';
				line+=tt::FNb(0,f1,3,false);
				ar.writeLine(line);
			}
		}
		ar.close();
