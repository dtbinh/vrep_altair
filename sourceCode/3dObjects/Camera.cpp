...
{ // drawText is false and passiveSubView is true by default
	...
	// Pass 1 : RENDERPASS : Pass which is always displayed and cannot be skipped
	// Pass 2 : PICKPASS : Pass used for picking
	//			This pass is never displayed and can be skipped
	int pass=RENDERPASS;
...
{ // -2 means: handle no mouse up hits. Otherwise, handle mouse up hits!
	...
	else
	{
		...
		{
			int dummy;
			int hitId=getSingleHit(hits,selectBuff,(App::ct->objCont->getEditModeType()&VERTEX_EDIT_MODE)||(App::ct->objCont->getEditModeType()&EDGE_EDIT_MODE),dummy);
			if (hitId==-1)
			{
				if ((App::ct->objCont->getEditModeType()&SHAPE_OR_PATH_EDIT_MODE)==0)
					App::ct->objCont->deselectObjects();
		...
		{ // no ctrl or shift key down here. We simply return the hit for processing at a later stage
			...
			int hitId=getSingleHit(hits,selectBuff,(App::ct->objCont->getEditModeType()&VERTEX_EDIT_MODE)||(App::ct->objCont->getEditModeType()&EDGE_EDIT_MODE),hitThatIgnoresTheSelectableFlag);
