...
CBezierPathPoint* CPathCont::_addBezierPathPoint(const C7Vector& transf,float maxRelAbsVelocity,float onSpotDistance)
{
...
void CPathCont::_recomputeBezierPoints()
{
	...
	for (int i=0;i<int(_simplePathPoints.size());i++)
	{
		...
		{
			if (itb==NULL)
				_addBezierPathPoint(itm->getTransformation(),itm->getMaxRelAbsVelocity(),itm->getOnSpotDistance()); // first point, open path
			else
				_addBezierPathPoint(itm->getTransformation(),itm->getMaxRelAbsVelocity(),itm->getOnSpotDistance()); // last point, open path
		}
		else
		{
			...
			if (itm->getBezierPointCount()!=1)
			{
				...
				if (linOk||angOk||onSpotDist)
				{
					...
					_addBezierPathPoint(tri,mxRelVel_i,onSpot_i); // first point in the bezier curve section
					...
					for (int k=1;k<divCount;k++)
					{
						...
						_addBezierPathPoint(trpt,mxRelVel_trpt,onSpot_trpt);
					}
					// We add the last point:
					_addBezierPathPoint(trj,mxRelVel_j,onSpot_j); // last point in the Bezier curve section
				}
			}
			if ( (!linOk)&&(!angOk)&&(!onSpotDist) )
			{ // This means we either have Bezier interpol. disabled or ((coincident points or colinear points)and(two same orientations))! 
				_addBezierPathPoint(tr1,itm->getMaxRelAbsVelocity(),itm->getOnSpotDistance());
			}
		}
	}
}
