...
{ // set the joint type directly after that!
	...
	_dynamicMotorPositionControl=false;
...
void CJoint::setHybridFunctionality(bool h)
{
	...
	if (h)
	{
		...
		_dynamicMotorPositionControl=true;
		if (_jointType==sim_joint_revolute_subtype)
		{
			_screwPitch=0.0f;
...
void CJoint::setEnableDynamicMotorPositionControl(bool p)
{
	_dynamicMotorPositionControl=p;
}

bool CJoint::getEnableDynamicMotorPositionControl()
{
	return(_dynamicMotorPositionControl);
}
...
{ // is called at simulation start, but also after object(s) have been copied into a scene!
	...
	if (simulationIsRunning)
	{
		...
		_initialDynamicMotorPositionControl=_dynamicMotorPositionControl;
...
{ // Remember, this is not guaranteed to be run! (the object can be copied during simulation, and pasted after it ended). For thoses situations there is the initializeInitialValues routine!
	if (_initialValuesInitialized&&App::ct->simulation->getResetSceneAtSimulationEnd()&&((getCumulativeMainPropertyOverride()&sim_modelproperty_not_reset)==0))
	{
		...
		_dynamicMotorPositionControl=_initialDynamicMotorPositionControl;
...
void CJoint::handleDynJointControl(bool init,int loopCnt,int totalLoops,float currentPos,float effort,float dynStepSize,float errorV,float& velocity,float& forceTorque)
{
	...
	{ // The plugins didn't want to handle that joint
		if (!_dynamicMotorCustomControl)
		{ // we have the built-in control (PID)
			if (init)
			{
				_dynamicMotorPIDCumulativeErrorForIntegralParameter=0.0f;
				_dynamicMotorPIDLastErrorForDerivativeParameter=0.0f;
			}

			float e=errorV;

			// Proportional part:
			float ctrl=e*_dynamicMotorPositionControl_P;

			// Integral part:
			if (_dynamicMotorPositionControl_I!=0.0f) // so that if we turn the integral part on, we don't try to catch up all the past errors!
				_dynamicMotorPIDCumulativeErrorForIntegralParameter+=e;
			else
				_dynamicMotorPIDCumulativeErrorForIntegralParameter=0.0f; // added on 2009/11/29
			ctrl+=_dynamicMotorPIDCumulativeErrorForIntegralParameter*_dynamicMotorPositionControl_I;

			// Derivative part:
			ctrl+=(e-_dynamicMotorPIDLastErrorForDerivativeParameter)*_dynamicMotorPositionControl_D;
			...
			//	printf("Error: %f, force: %f, vel: %f\n",errorV,forceTorque,velocity);
		}
		else
...
C3DObject* CJoint::copyYourself()
{
	...
	newJoint->_dynamicMotorPositionControl=_dynamicMotorPositionControl;
	...
	newJoint->_initialDynamicMotorPositionControl=_initialDynamicMotorPositionControl;
...
void CJoint::serialize(CSer& ar)
{
	...
	{	// Storing. The order in which we are storing is very important!!!!
		...
		SIM_SET_CLEAR_BIT(dummy,5,_dynamicMotorPositionControl);
		...
		while (theName.compare(SER_END_OF_OBJECT)!=0)
		{
			...
			if (theName.compare(SER_END_OF_OBJECT)!=0)
			{
				...
				if (theName.compare("Va9")==0)
				{
					...
					_dynamicMotorPositionControl=SIM_IS_BIT_SET(dummy,5);
...
void CJoint::setJointMode(int theMode)
{
	...
	if ((theMode==sim_jointmode_force)&&_dynamicMotorPositionControl)
