									  ...
									  char* charData,int& charDataPos)
{
	...
	for (int i=0;i<dataSize;i++)
	{
		...
		{
			lua_pushstring(L,charData+charDataPos);
			charDataPos+=strlen(charData+charDataPos)+1; // Thanks to Ulrich Schwesinger for noticing a bug here!
		}
		else
...
{	// THIS FUNCTION SHOULD NOT LOCK THE API (AT LEAST NOT WHILE CALLING THE CALLBACK!!) SINCE IT IS NOT DIRECTLY ACCESSING THE API!!!!
	...
	{ // we now search for the callback to call:
		...
		{ // we have the right one! Now we need to prepare the input and output argument arrays:
			...
			{
				if (!readCustomFunctionDataFromStack(L,i+1,it->inputArgTypes[i],inBoolVector,inIntVector,inFloatVector,inStringVector,inInfoVector))
					break;
			}
...
int _simTest(lua_State* L)
{
	LUA_API_FUNCTION_DEBUG;
	LUA_START("simTest");
	CThreadPool::setThreadAutomaticSwitch(luaToBool(L,1));
	LUA_END(0);
}
...
int _simJointGetForce(lua_State* L)
{
	...
	{
		float jointF[1];
		if (simJointGetForce_internal(luaToInt(L,1),jointF)!=-1)
				...
				else
				{
					...
					if (!argError)
					{
						...
						while (!movementFinished)
						{
							...
							int rmlRes=simRMLPosition_internal(4,dt,flags,currentPosVelAccel,maxVelAccelJerk,selection,targetPosVel,newPosVelAccel,auxData);
			...
			{ 
				...
				if (!argError)
				{
					...
					for (int i=0;i<dofs;i++)
					{
						...
						{
							selection[i]=1;
							currentPosVelAccel[0*dofs+i]=act->getPosition();
						}
						else
							selection[i]=0;
...
int _simFindMpPath(lua_State* L)
{
	...
	if (checkInputArguments(L,functionName,true,lua_arg_number,0,lua_arg_number,1,lua_arg_number,1,lua_arg_number,0,lua_arg_number,0))
	{
		...
		if (it!=NULL)
		{
			...
			if (checkInputArguments(L,functionName,true,lua_arg_number,0,lua_arg_number,it->getJointCount(),lua_arg_number,it->getJointCount(),lua_arg_number,0,lua_arg_number,0))
			{
				std::vector<float> startConfig(it->getJointCount(),0.0f);
				std::vector<float> goalConfig(it->getJointCount(),0.0f);
				getFloatsFromTable(L,2,it->getJointCount(),&startConfig[0]);
				getFloatsFromTable(L,3,it->getJointCount(),&goalConfig[0]);
				...
				if (res>=0)
				{
					...
					if (configs!=NULL)
					{
						pushFloatTableOntoStack(L,it->getJointCount()*configCount,configs);
						pushFloatTableOntoStack(L,configCount,configs+it->getJointCount()*configCount);
						pushFloatTableOntoStack(L,configCount*3,configs+it->getJointCount()*configCount+configCount);
						LUA_END(3);
					}
				}
			}
...
{
    LUA_API_FUNCTION_DEBUG;
    LUA_START("simAttachDirtyTexture");
    if (checkInputArguments(L,functionName,true,lua_arg_number,0,lua_arg_string,0))
    {
        int headerNumber=luaToInt(L,1);
        size_t dataLength;
        char* data=(char*)lua_tolstring(L,2,&dataLength);
        int retval = simAttachDirtyTexture_internal(headerNumber, data);
        lua_pushnumber(L,retval);
    }
    LUA_END(1);
}
