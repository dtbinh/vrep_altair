...
void CPathPlanningInterface::setMpObjectData(CDummyMpObject* mpObject,int jointCnt,const int* jointHandles,const int* jointStepCount,int ikGroupId,int baseFrameId,int tipFrameId,int robotSelfCollEntity1,int robotSelfCollEntity2,int robotEntity,int obstacleEntity,float distanceThreshold,const float* tipMetric,const float* robotMetric)
{
#ifndef COMPILING_EXTERNAL_PATHPLANNING_DLL
	if (!initializeFunctionsIfNeeded())
#endif
#ifdef COMPILING_EXTERNAL_PATHPLANNING_DLL
	{ // we use the internal functions
		((CmpObject*)mpObject)->setData(jointCnt,jointHandles,jointStepCount,ikGroupId,baseFrameId,tipFrameId,robotSelfCollEntity1,robotSelfCollEntity2,robotEntity,obstacleEntity,distanceThreshold,tipMetric,robotMetric);
		return;
	}
#else
		return;
...
#ifndef COMPILING_EXTERNAL_PATHPLANNING_DLL
	// we call the plugin function:
	_setMpObjectData(mpObject,jointCnt,jointHandles,jointStepCount,ikGroupId,baseFrameId,tipFrameId,robotSelfCollEntity1,robotSelfCollEntity2,robotEntity,obstacleEntity,distanceThreshold,tipMetric,robotMetric);
#endif
}
