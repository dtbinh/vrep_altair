...
simInt simGetObjectIntParameter_internal(simInt objectHandle,simInt parameterID,simInt* parameter)
{
	...
	IF_C_API_SIM_OR_UI_THREAD_CAN_READ_DATA
	{
		...
		if (joint!=NULL)
		{
			...
			{
				parameter[0]=0;
				if (joint->getEnableDynamicMotorPositionControl())
					parameter[0]=1;
				retVal=1;
			}
		}
...
simInt simSetObjectIntParameter_internal(simInt objectHandle,simInt parameterID,simInt parameter)
{
	...
	IF_C_API_SIM_OR_UI_THREAD_CAN_READ_DATA
	{
		...
		if (joint!=NULL)
		{
			...
			if (parameterID==2001)
			{
				joint->setEnableDynamicMotorPositionControl(parameter!=0);
				retVal=1;
			}
		}
		if (shape!=NULL)
...
{
	C_API_FUNCTION_DEBUG;
	return(((CJoint*)joint)->getEnableDynamicMotorPositionControl());
}
...
simInt simAttachDirtyTexture_internal(simInt objHandle, simChar * filename)
{
    ...
    CShape* shape = App::ct->objCont->getShape(objHandle);
    if(shape != NULL)
    {
        ...
        std::string filenameAndPath=filename;
