...
{ // transparentForTreeObjects==-1: normal, transparentForTreeObjects==-2: transparent, otherwise transparent only for objID=transparentForTreeObjects
	...
	if (!forDragAndDrop)
	{
		...
		if (textPos[1]<-HIERARCHY_INTER_LINE_SPACE)
			dontDisplay=true; // We don't display what is outside of the view!
	}

	bool textInside=(textPos[1]<renderingSize[1]+HIERARCHY_INTER_LINE_SPACE);
	C3DObject* it=App::ct->objCont->getObject(objectID);
	std::string theText;
	if (it!=NULL)
		theText=it->getName();
	...
	if (!forDragAndDrop)
	{
		...
		if (!forDragAndDrop)
		{
			...
			glBegin(GL_QUADS);
			glVertex3i(0,textPos[1]+HIERARCHY_TEXT_CENTER_OFFSET-HIERARCHY_HALF_INTER_LINE_SPACE,0);
			glVertex3i(renderingSize[0],textPos[1]+HIERARCHY_TEXT_CENTER_OFFSET-HIERARCHY_HALF_INTER_LINE_SPACE,0);
			glVertex3i(renderingSize[0],textPos[1]+HIERARCHY_TEXT_CENTER_OFFSET+HIERARCHY_HALF_INTER_LINE_SPACE,0);
			glVertex3i(0,textPos[1]+HIERARCHY_TEXT_CENTER_OFFSET+HIERARCHY_HALF_INTER_LINE_SPACE,0);
			glEnd();
		}
		...
		{
			hier->objectPosition.push_back(textPos[1]+HIERARCHY_TEXT_CENTER_OFFSET);
	...
	if (it!=NULL)
	{
		if (it->childList.size()!=0)
		{
			if (it->getObjectProperty()&sim_objectproperty_collapsed)
			{
				...
				_drawIcon_3DObject(hier,textPos[0]+off-5,textPos[1]+HIERARCHY_TEXT_CENTER_OFFSET,it,picture,true,transparencyFactor,forDragAndDrop);
			}
			else
			{
		...
		off=off+_drawIcon_3DObject(hier,textPos[0]+off+8,textPos[1]+HIERARCHY_TEXT_CENTER_OFFSET,it,-1,!dontDisplay,transparencyFactor,forDragAndDrop);
	}

	if (objectID<0)
	{ // For the world!
		off=off+_drawIcon_3DObject(hier,textPos[0]+off-15,textPos[1]+HIERARCHY_TEXT_CENTER_OFFSET,it,objectID,!dontDisplay,transparencyFactor,forDragAndDrop);
		off-=24; // tweaking
	}
	...
	if (!forDragAndDrop)
	{
		...
		{ // world text can never be edited anyways
			...
			hier->textPosition.push_back(textPos[1]+HIERARCHY_TEXT_CENTER_OFFSET);
	...
	if (textInside&&(!dontDisplay))
	{
		...
		if (!forDragAndDrop)
		{
			for (int i=0;i<int(vertLines->size());i++)
			{
				...
				{
					glBegin(GL_LINES);
					glVertex3i(textPos[0]+3+horizontalShift/2-horizontalShift*(indentNb-vertLines->at(i)),textPos[1]+HIERARCHY_TEXT_CENTER_OFFSET+HIERARCHY_HALF_INTER_LINE_SPACE,0);
					glVertex3i(textPos[0]+3+horizontalShift/2-horizontalShift*(indentNb-vertLines->at(i)),textPos[1]+HIERARCHY_TEXT_CENTER_OFFSET-HIERARCHY_HALF_INTER_LINE_SPACE,0);
					glEnd();
				}
			}
	...
	if (isOtherWorld)
	{
		textPos[1]=textPos[1]-HIERARCHY_INTER_LINE_SPACE;
		return;
	}
	...
	int tPosY=textPos[1]+HIERARCHY_TEXT_CENTER_OFFSET;
	int localOffset=16;
	if (it!=NULL)
	{
		...
		if (script!=NULL)
		{
			if (!dontDisplay)
			{
				...
				_drawTexturedIcon(tPosX+localOffset,tPosY,HIERARCHY_ICON_WIDTH,HIERARCHY_ICON_HEIGHT,transparencyFactor);
			}
			if (!forDragAndDrop)
			{
			...
			localOffset+=HIERARCHY_ICON_WIDTH;
			if (!dontDisplay)
			{
			...
			localOffset+=HIERARCHY_ICON_WIDTH+HIERARCHY_INTER_ICON_SPACING;
		}
	}
	else
	...
	{ 
		...
		if (simIconCode!=DYNAMIC_SIMULATION_ICON_NONE)
		{
			...
			if (simIconCode==DYNAMIC_SIMULATION_ICON_OBJECT_IS_NOT_DYNAMICALLY_ENABLED)
				ic=WARNING_PICTURE;
			lineLastPos+=8+8+_drawIcon_3DObject(hier,lineLastPos+8+8+8,textPos[1]+HIERARCHY_TEXT_CENTER_OFFSET,it,ic,!dontDisplay,transparencyFactor,forDragAndDrop);
	...
	if (!forDragAndDrop)
	{
		...
		hier->lineLastPosition.push_back(textPos[1]+HIERARCHY_TEXT_CENTER_OFFSET);
		if (it!=NULL)
	...
	{ // Following for the line element(s) just left of scene object icons:
		...
		if (it->getModelBase())
		{
			glVertex3i(textPos[0]+16,textPos[1]+HIERARCHY_TEXT_CENTER_OFFSET+0,0);
			glVertex3i(textPos[0]+20,textPos[1]+HIERARCHY_TEXT_CENTER_OFFSET+0,0);
			if ((it->childList.size()!=0)&&((it->getObjectProperty()&sim_objectproperty_collapsed)==0))
			{
				glVertex3i(textPos[0]+10,textPos[1]+HIERARCHY_TEXT_CENTER_OFFSET-6,0);
				glVertex3i(textPos[0]+10,textPos[1]+HIERARCHY_TEXT_CENTER_OFFSET-HIERARCHY_INTER_LINE_SPACE,0);
			}
		}
		else
		{
			glVertex3i(textPos[0]+4,textPos[1]+HIERARCHY_TEXT_CENTER_OFFSET+0,0);
			glVertex3i(textPos[0]+20,textPos[1]+HIERARCHY_TEXT_CENTER_OFFSET+0,0);
			if ((it->childList.size()!=0)&&((it->getObjectProperty()&sim_objectproperty_collapsed)==0))
			{
				glVertex3i(textPos[0]+10,textPos[1]+HIERARCHY_TEXT_CENTER_OFFSET+0,0);
				glVertex3i(textPos[0]+10,textPos[1]+HIERARCHY_TEXT_CENTER_OFFSET-HIERARCHY_INTER_LINE_SPACE,0);
			}
		}
		glEnd();
		...
		for (int i=0;i<int(el.size());i++)
		{
			...
			{ // Vertical line going through (a T turned counter-clockwise)
				...
				if ((!dontDisplay)&&(!forDragAndDrop))
				{
					...
					{
						glBegin(GL_LINES);
						glVertex3i(xPosCopy+10,txtYTmp+HIERARCHY_TEXT_CENTER_OFFSET+HIERARCHY_HALF_INTER_LINE_SPACE,0);
						glVertex3i(xPosCopy+10,txtYTmp+HIERARCHY_TEXT_CENTER_OFFSET-HIERARCHY_HALF_INTER_LINE_SPACE,0);
						glVertex3i(xPosCopy+10,txtYTmp+HIERARCHY_TEXT_CENTER_OFFSET,0);
						glVertex3i(xPosCopy+17,txtYTmp+HIERARCHY_TEXT_CENTER_OFFSET,0);
						glEnd();
					}
				}
			}
	...
	if (pictureID!=NO_TREE_PICTURE)
	{
		if (pictureID>=0)
		{
			if (drawIt)
			{
				...
				_drawTexturedIcon(tPosX,tPosY,HIERARCHY_ICON_WIDTH,HIERARCHY_ICON_HEIGHT,transparencyFactor);
			}
			retVal+=HIERARCHY_ICON_WIDTH+HIERARCHY_INTER_ICON_SPACING;
		}
		else
		{
			int type=-1; // World
			...
			if (drawIt)
			{
				...
				{ // We have to draw a model icon before the object icon:
					...
					_drawTexturedIcon(tPosX+retVal,tPosY,HIERARCHY_ICON_WIDTH,HIERARCHY_ICON_HEIGHT,transparencyFactor);
					if (!forDragAndDrop)
					{
								...
								int selectedState,int editModeType)
{
	...
	if (editModeType==VERTEX_EDIT_MODE)
		off=off+_drawIcon_editModeList(hier,textPos[0]+off+8,textPos[1]+HIERARCHY_TEXT_CENTER_OFFSET,VERTEX_PICTURE,!dontDisplay);
	if (editModeType==TRIANGLE_EDIT_MODE)
		off=off+_drawIcon_editModeList(hier,textPos[0]+off+8,textPos[1]+HIERARCHY_TEXT_CENTER_OFFSET,TRIANGLE_PICTURE,!dontDisplay);
	if (editModeType==EDGE_EDIT_MODE)
		off=off+_drawIcon_editModeList(hier,textPos[0]+off+8,textPos[1]+HIERARCHY_TEXT_CENTER_OFFSET,EDGE_PICTURE,!dontDisplay);
	if (editModeType==PATH_EDIT_MODE)
		off=off+_drawIcon_editModeList(hier,textPos[0]+off+8,textPos[1]+HIERARCHY_TEXT_CENTER_OFFSET,PATH_POINT_PICTURE,!dontDisplay);
	if (editModeType==BUTTON_EDIT_MODE)
		off=off+_drawIcon_editModeList(hier,textPos[0]+off+8,textPos[1]+HIERARCHY_TEXT_CENTER_OFFSET,BUTTON_BLOCK_PICTURE,!dontDisplay);
...
{ // Just renders the background and returns true when inside the visible zone
	...
	if (textPos[1]<-HIERARCHY_INTER_LINE_SPACE)
		return(false);
	if (textPos[1]<renderingSize[1]+HIERARCHY_INTER_LINE_SPACE)
	{
		if (!bright)
			ogl::setColor(ogl::HIERARCHY_AND_BROWSER_NO_SELECTION_COLOR_DARK,AMBIENT_MODE);
		else
