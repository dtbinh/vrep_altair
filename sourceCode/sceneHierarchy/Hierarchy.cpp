...
{
	renderingSize[0]=xSize+SAFETY_BORDER_SIZE;
	renderingSize[1]=ySize+SAFETY_BORDER_SIZE;
	renderingPosition[0]=xPos-SAFETY_BORDER_SIZE;
	renderingPosition[1]=yPos-SAFETY_BORDER_SIZE;
...
void CHierarchy::keyPress(int key)
{
	...
	{ // Normal mode (no label edition)
		if (key==UP_KEY)
			viewPosition[1]=viewPosition[1]+HIERARCHY_INTER_LINE_SPACE;
		if (key==DOWN_KEY)
			viewPosition[1]=viewPosition[1]-HIERARCHY_INTER_LINE_SPACE;
		if (key==LEFT_KEY)
			viewPosition[0]=viewPosition[0]+HIERARCHY_INTER_LINE_SPACE;
		if (key==RIGHT_KEY)
			viewPosition[0]=viewPosition[0]-HIERARCHY_INTER_LINE_SPACE;
...
{ // return value true means the hierarchy was refreshed
	...
	if (editModeType==NO_EDIT_MODE)
	{
		...
		if (hierarchDragUnderway)
		{
			...
			if (ct-dragStartTime>1000)
			{
				...
				{
					if (mouseRelativePosition[1]<renderingPosition[1]+BROWSER_HIERARCHY_TITLE_BAR_HEIGHT+40+HIERARCHY_INTER_LINE_SPACE*4) // Bottom border
						viewPosition[1]=viewPosition[1]+HIERARCHY_INTER_LINE_SPACE;
					if (mouseRelativePosition[1]<renderingPosition[1]+BROWSER_HIERARCHY_TITLE_BAR_HEIGHT+40+HIERARCHY_INTER_LINE_SPACE*2) // Bottom border
						viewPosition[1]=viewPosition[1]+HIERARCHY_INTER_LINE_SPACE;
					if (mouseRelativePosition[1]>renderingPosition[1]+BROWSER_HIERARCHY_TITLE_BAR_HEIGHT+renderingSize[1]-HIERARCHY_INTER_LINE_SPACE*4) // top border
						viewPosition[1]=viewPosition[1]-HIERARCHY_INTER_LINE_SPACE;
					if (mouseRelativePosition[1]>renderingPosition[1]+BROWSER_HIERARCHY_TITLE_BAR_HEIGHT+renderingSize[1]-HIERARCHY_INTER_LINE_SPACE*2) // top border
						viewPosition[1]=viewPosition[1]-HIERARCHY_INTER_LINE_SPACE;

					if (mouseRelativePosition[0]<HIERARCHY_INTER_LINE_SPACE*2) // left border
						viewPosition[0]=viewPosition[0]+HIERARCHY_INTER_LINE_SPACE;
					if (mouseRelativePosition[0]>renderingSize[0]-HIERARCHY_INTER_LINE_SPACE*2) // right border
						viewPosition[0]=viewPosition[0]-HIERARCHY_INTER_LINE_SPACE;
	...
	glScissor(renderingPosition[0]+SAFETY_BORDER_SIZE,renderingPosition[1]+BROWSER_HIERARCHY_TITLE_BAR_HEIGHT,renderingSize[0]-SAFETY_BORDER_SIZE,renderingSize[1]);
	...
	glLoadIdentity ();
	glDisable(GL_DEPTH_TEST);
	glScissor(renderingPosition[0]+SAFETY_BORDER_SIZE,renderingPosition[1],renderingSize[0]-SAFETY_BORDER_SIZE,renderingSize[1]);
	...
	if (shiftSelectionStarted)
	{
		...
		if (limitedPos[0]<SAFETY_BORDER_SIZE)
			limitedPos[0]=SAFETY_BORDER_SIZE;
		if (limitedPos[1]<SAFETY_BORDER_SIZE)
			limitedPos[1]=SAFETY_BORDER_SIZE;
...
//	float black[3]={0.0f,0.0f,0.0f};
	...
	{ // here we draw the horizontal slider:
		ogl::setColor(ogl::HIERARCHY_AND_BROWSER_SCROLLBAR_BACK_COLOR,AMBIENT_MODE);
		int leftTop[2]={0,horizontalScrollbarHeight+SAFETY_BORDER_SIZE};
		int rightBottom[2]={renderingSize[0],0};
		glBegin(GL_QUADS);
		glVertex3i(leftTop[0],leftTop[1],0);
		glVertex3i(rightBottom[0],leftTop[1],0);
...
int CHierarchy::getSliderPositions(int vSliderTopLeft[2],int vSliderBottomRight[2],int hSliderTopLeft[2],int hSliderBottomRight[2],float prop[2])
{
	...
	{ // we need to do it 3 times because one added slider might mean we have to add the other one!
		...
		if (minMaxViewPosition[0]!=0)
			horizontalScrollbarHeight=HIERARCHY_SCROLLBAR_WIDTH;
		if (minMaxViewPosition[1]!=0)
			verticalScrollbarWidth=HIERARCHY_SCROLLBAR_WIDTH;
	}
	int retVal=0;
	if (verticalScrollbarWidth>0)
	{ // here we compute the vertical slider:
		retVal|=1;
		int effDr=renderingSize[1]-horizontalScrollbarHeight-SAFETY_BORDER_SIZE;//-4;
		int sliderS=effDr*effDr/(effDr+minMaxViewPosition[1]);
		if (sliderS<HIERARCHY_SCROLLBAR_WIDTH)
			sliderS=HIERARCHY_SCROLLBAR_WIDTH;
	...
	{ // here we compute the horizontal slider:
		retVal|=2;
		int effDr=renderingSize[0]-verticalScrollbarWidth-SAFETY_BORDER_SIZE-4;
		...
		hSliderTopLeft[1]=horizontalScrollbarHeight+SAFETY_BORDER_SIZE;
		hSliderBottomRight[1]=0;
		hSliderTopLeft[0]=effDr-sliderS-sliderP+SAFETY_BORDER_SIZE;
...
bool CHierarchy::leftMouseDown(int x,int y,int selectionStatus)
{
	...
	if (y<0)
		return(false);
	if (x>renderingSize[0]-SAFETY_BORDER_SIZE)
		return(false);
	if (y>renderingSize[1]-SAFETY_BORDER_SIZE)
		return(false);
	// The mouse went down on the hierarchy window!
	_caughtElements|=sim_left_button;

	refreshViewFlag=3;
	mouseDownRelativePosition[0]=x+SAFETY_BORDER_SIZE;
	mouseDownRelativePosition[1]=y+SAFETY_BORDER_SIZE;
	mouseRelativePosition[0]=x+SAFETY_BORDER_SIZE;
	mouseRelativePosition[1]=y+SAFETY_BORDER_SIZE;
	previousMouseRelativePosition[0]=x+SAFETY_BORDER_SIZE;
	previousMouseRelativePosition[1]=y+SAFETY_BORDER_SIZE;
	shiftSelectionStarted=false;
	labelEditObjectID=-1;
	...
	{ // NO EDIT MODE
		...
		{
			int objID=getActionObjectID(mouseDownRelativePosition[1],HIERARCHY_HALF_INTER_LINE_SPACE);
			if (objID>=0)
			{
...
{
	int dx=(x+SAFETY_BORDER_SIZE)-mouseDownRelativePosition[0];
	int dy=(y+SAFETY_BORDER_SIZE)-mouseDownRelativePosition[1];
	...
	{ // SHIFT SELECTION
		...
		if (limitedPos<SAFETY_BORDER_SIZE)
			limitedPos=SAFETY_BORDER_SIZE;
		std::vector<int> objToBeSelected;
		getActionObjectIDs(mouseDownRelativePosition[1],limitedPos,HIERARCHY_HALF_INTER_LINE_SPACE,&objToBeSelected);
		if (App::ct->objCont->getEditModeType()==NO_EDIT_MODE)
		{
	...
	if ((_worldSelectID_down!=-9999)&&(_worldSelectID_down==getActionObjectID(mouseRelativePosition[1],HIERARCHY_HALF_INTER_LINE_SPACE)))
	{
...
{ // Only caught if right button was caught by the hierarchy!
	...
	if ( (x<0)||(y<0)||(x>renderingSize[0]-SAFETY_BORDER_SIZE)||(y>renderingSize[1]-SAFETY_BORDER_SIZE) )
		return;
...
bool CHierarchy::mouseWheel(int deltaZ,int x,int y)
{
	if ( (x<0)||(y<0)||(x>renderingSize[0]-SAFETY_BORDER_SIZE)||(y>renderingSize[1]-SAFETY_BORDER_SIZE) )
		return(false);
	refreshViewFlag=3;
	viewPosition[1]=viewPosition[1]-(deltaZ/120)*HIERARCHY_INTER_LINE_SPACE;
	return(true);
}
...
void CHierarchy::mouseMove(int x,int y,bool passiveAndFocused)
{
	...
	if (!passiveAndFocused)
	{
		...
		{
			refreshViewFlag=3;
			int objID=getActionObjectID(mouseRelativePosition[1],HIERARCHY_HALF_INTER_LINE_SPACE);
			if (objID<0)
				_worldSelectID_moving=objID;
		}
			...
			if (it!=NULL)
			{
				...
				if (it->getDynamicSimulationIconCode()==DYNAMIC_SIMULATION_ICON_OBJECT_IS_DYNAMICALLY_SIMULATED)
				{
					...
					if (it->getObjectType()==sim_object_joint_type)
					{
						...
						txt+=' ';
						if (so->getEnableDynamicMotorPositionControl())
							txt+=IDS__POSITION_CONTROL_;
						else
							txt+=IDS__VELOCITY_FORCE_TORQUE_CONTROL_;
					}
...
{
	int tmp=renderingSize[0]-maxRenderedPosition[0]-viewPosition[0]-SAFETY_BORDER_SIZE-verticalScrollbarWidth;
	if (tmp>0)
		viewPosition[0]=renderingSize[0]-(maxRenderedPosition[0]-minRenderedPosition[0])-SAFETY_BORDER_SIZE-verticalScrollbarWidth;
	if (viewPosition[0]>SAFETY_BORDER_SIZE)
		viewPosition[0]=SAFETY_BORDER_SIZE;
	tmp=renderingSize[1]-(maxRenderedPosition[1]-minRenderedPosition[1])+viewPosition[1]-SAFETY_BORDER_SIZE-4-horizontalScrollbarHeight;
	if (tmp>0)
		viewPosition[1]=maxRenderedPosition[1]-minRenderedPosition[1]-renderingSize[1]+SAFETY_BORDER_SIZE+6+horizontalScrollbarHeight;
	if (viewPosition[1]<-SAFETY_BORDER_SIZE+8)
		viewPosition[1]=-SAFETY_BORDER_SIZE+8;
}
...
int CHierarchy::getInflateActionObjectID(int mousePositionX,int mousePositionY)
{
	...
	{
		if ((mousePositionX>=inflateIconPosition[3*i+0]-HIERARCHY_ICON_QUARTER_WIDTH)&&(mousePositionX<=inflateIconPosition[3*i+0]+HIERARCHY_ICON_QUARTER_WIDTH) )
		{
			if ((mousePositionY>=inflateIconPosition[3*i+1]-HIERARCHY_ICON_QUARTER_HEIGHT)&&(mousePositionY<=inflateIconPosition[3*i+1]+HIERARCHY_ICON_QUARTER_HEIGHT) )
...
int CHierarchy::getScriptActionObjectID(int mousePositionX,int mousePositionY)
{
	...
	{
		if ((mousePositionX>=scriptIconPosition[3*i+0]-HIERARCHY_ICON_HALF_WIDTH)&&(mousePositionX<=scriptIconPosition[3*i+0]+HIERARCHY_ICON_HALF_WIDTH) )
		{
			if ((mousePositionY>=scriptIconPosition[3*i+1]-HIERARCHY_ICON_HALF_HEIGHT)&&(mousePositionY<=scriptIconPosition[3*i+1]+HIERARCHY_ICON_HALF_HEIGHT) )
...
int CHierarchy::getScriptParameterActionObjectID(int mousePositionX,int mousePositionY)
{
	...
	{
		if ((mousePositionX>=scriptParametersIconPosition[3*i+0]-HIERARCHY_ICON_HALF_WIDTH)&&(mousePositionX<=scriptParametersIconPosition[3*i+0]+HIERARCHY_ICON_HALF_WIDTH) )
		{
			if ((mousePositionY>=scriptParametersIconPosition[3*i+1]-HIERARCHY_ICON_HALF_HEIGHT)&&(mousePositionY<=scriptParametersIconPosition[3*i+1]+HIERARCHY_ICON_HALF_HEIGHT) )
...
int CHierarchy::getTextActionObjectID(int mousePositionX,int mousePositionY)
{
	for (int i=0;i<int(textPosition.size())/6;i++)
	{
		...
		{
			if ((mousePositionY>=textPosition[6*i+4]-HIERARCHY_HALF_INTER_LINE_SPACE)&&(mousePositionY<=textPosition[6*i+4]+HIERARCHY_HALF_INTER_LINE_SPACE) )
...
int CHierarchy::getActionObjectID_icon(int mousePositionX,int mousePositionY,bool ignoreXPositionAndCheckLineInstead/*=false*/)
{
	if (!ignoreXPositionAndCheckLineInstead)
	{
		...
		{
			if ( (mousePositionX>=objectIconPosition[3*i+0]-HIERARCHY_ICON_HALF_WIDTH)&&(mousePositionX<=objectIconPosition[3*i+0]+HIERARCHY_ICON_HALF_WIDTH) )
			{
				if ((mousePositionY>=objectIconPosition[3*i+1]-HIERARCHY_ICON_HALF_HEIGHT)&&(mousePositionY<=objectIconPosition[3*i+1]+HIERARCHY_ICON_HALF_HEIGHT) )
	...
	else
	{
		for (int i=0;i<int(objectIconPosition.size())/3;i++)
		{
			if ((mousePositionY>=objectIconPosition[3*i+1]-HIERARCHY_HALF_INTER_LINE_SPACE)&&(mousePositionY<=objectIconPosition[3*i+1]+HIERARCHY_HALF_INTER_LINE_SPACE) )
				return(objectIconPosition[3*i+2]);
		}
	}
	return(-9999); // minus number are for worlds (also in different instances)
...
int CHierarchy::getActionModelID_icon(int mousePositionX,int mousePositionY)
{
	...
	{
		if ((mousePositionX>=modelIconPosition[3*i+0]-HIERARCHY_ICON_QUARTER_WIDTH)&&(mousePositionX<=modelIconPosition[3*i+0]+HIERARCHY_ICON_QUARTER_WIDTH) )
		{
			if ((mousePositionY>=modelIconPosition[3*i+1]-HIERARCHY_ICON_QUARTER_HEIGHT)&&(mousePositionY<=modelIconPosition[3*i+1]+HIERARCHY_ICON_QUARTER_HEIGHT) )
...
int CHierarchy::getSimulationActionObjectID(int mousePositionX,int mousePositionY)
{
	...
	{
		if ((mousePositionX>=simulationIconPosition[3*i+0]-HIERARCHY_ICON_HALF_WIDTH)&&(mousePositionX<=simulationIconPosition[3*i+0]+HIERARCHY_ICON_HALF_WIDTH) )
		{
			if ((mousePositionY>=simulationIconPosition[3*i+1]-HIERARCHY_ICON_HALF_HEIGHT)&&(mousePositionY<=simulationIconPosition[3*i+1]+HIERARCHY_ICON_HALF_HEIGHT) )
...
void CHierarchy::drawEditionLabel(int textPosX,int textPosY)
{
	...
	VPoint p(textPosX-2+buttonWidth/2,textPosY+HIERARCHY_TEXT_CENTER_OFFSET);
	VPoint s(buttonWidth,HIERARCHY_INTER_LINE_SPACE);
