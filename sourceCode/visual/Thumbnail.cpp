...
void CThumbnail::setUncompressedThumbnailImage(const char* uncompressedRGBAImage)
{
	setRandomImage();
...
CThumbnail* CThumbnail::copyYourself()
{
	...
	if (_thumbnailRGBAImage!=NULL)
		newT->setUncompressedThumbnailImage(_thumbnailRGBAImage);
...
void CThumbnail::copyFrom(CThumbnail* it)
{
	...
	else
	{
		if (_thumbnailRGBAImage==NULL)
			_thumbnailRGBAImage=new char[128*128*4];
		setUncompressedThumbnailImage(it->_thumbnailRGBAImage);
	}
