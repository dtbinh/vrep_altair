...
void CDirBrowser::keyPress(int key)
{
	...
	refreshViewFlag=3;	
	if (key==UP_KEY)
		viewPosition[1]=viewPosition[1]+18;
	if (key==DOWN_KEY)
		viewPosition[1]=viewPosition[1]-18;
	if (key==LEFT_KEY)
		viewPosition[0]=viewPosition[0]+18;
	if (key==RIGHT_KEY)
		viewPosition[0]=viewPosition[0]-18;
	if (key==ESC_KEY)
	{
...
{ // return value true means the hierarchy was refreshed
	if (viewPosition[0]<-2000) // somehow there is a bug I can't put the finger on right now (2009/12/16)
		viewPosition[0]=0;
	if (resetViewFlag)
	{
		viewPosition[0]=0;
...
int CDirBrowser::getSliderPositions(int vSliderTopLeft[2],int vSliderBottomRight[2],int hSliderTopLeft[2],int hSliderBottomRight[2],float prop[2])
{
	...
	{ // here we compute the vertical slider:
		retVal|=1;
		int effDr=renderingSize[1]-horizontalScrollbarHeight-SAFETY_BORDER_SIZE;
